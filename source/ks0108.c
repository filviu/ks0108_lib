...
void GLCD_Initalize(void) {
     ...
     GLCD_WriteCommand((DISPLAY_ON_CMD | ON), 2);
}
...
void GLCD_WriteChar(char charToWrite) {
     ...
     charToWrite -= 32;
     for(i = 0; i < 5; i++)
           GLCD_WriteData(font5x7[charToWrite * 5 + i]);
     GLCD_WriteData(0x00);
}
...
void GLCD_Bitmap(code const unsigned char *bmp, unsigned char x, \
                      unsigned char y, unsigned char dx, unsigned char dy) {
